...
for i in range(len(D)-3):
	...
	for j in range(0,4):
		if i%20<17:
			side_product *= D[j+i]
	B.append(side_product)
	

for k in range(len(D)-60):
	down_product = 1 
	for l in range(0,4):
		down_product *= D[20*l+k]
	B.append(down_product)
	

for t in range(len(D)-63):
	right_diagonal_product = 1
	for s in range(0,4):
		if t%20<17:
			right_diagonal_product *= D[21*s+t]
	B.append(right_diagonal_product)

	
for p in range(60,len(D)-3):
